...
## 2026-07-12 (검증 안 된 risk 필드 삭제 + openFDA 라벨경고 접지)

- **D40. `risk` 한 줄 필드 삭제 + 부작용/경고는 openFDA SPL 경고를 verbatim 접지한다 (B-33 해결, 2026-07-12 사용자 확정)**: 발견 — `ingredients.json`의 `risk` 한 줄(17/17)은 Day-1 스캐폴딩(`c31f622`, Co-Authored-By Claude)에서 **LLM이 저작**했고 이후 검증 패스(ceiling/dosing만)에 포함된 적 없다 = **trust core에 숨은 미검증 LLM 의료 텍스트.** `verify:false`는 ceiling 문서화용이지 risk 검증이 아님. **결정: risk 필드를 삭제**(surface되면 논지 위반, 현재 dead). **부작용·경고 질문은 openFDA drug label의 OTC 경고섹션(warnings·do_not_use·stop_use·when_using·ask_doctor·ask_doctor_or_pharmacist)을 제품당 결정론 추출(scripts/fda-warnings.ts, fda-add 파이프라인 재사용)해 verbatim + set_id로 접지**하고, UI 카드(LabelWarnings)로 렌더한다. **LLM은 경고 텍스트를 절대 생성/패러프레이즈하지 않는다** — tool.ts는 경고 텍스트 미포함(포인터만), agent는 "표시된 라벨 경고를 가리키고 전체는 Drug Facts/약사로 defer, 기억 나열 금지." 근거: (1)삭제한 한 줄과 달리 SPL 경고는 실제 FDA 라벨 verbatim(set_id 인용), (2)부작용은 길고 종류 많아 한 줄 요약이 필연적 왜곡(사용자 지적) → 요약 말고 원문+전체라벨 링크, (3)제품당(SPL)이라 성분당이던 옛 필드와 구조가 다름. 커버리지 46/47(aleve는 SPL 경고섹션 부재→null, 지어내기 안 함). verify() 판정 코어 불변, additive. cf. B-33. 사용자(약사) 확정 2026-07-12.

## 2026-07-12 (프론트엔드 재빌드 — Traffic-Light 방향)

- **D41. 프론트엔드 디자인 방향 = Traffic-Light Safety (B), 사용자 확정 2026-07-12**: answer-engine·헬스앱·의료신뢰 디자인 조사 → 10방향 목업(아티팩트) → 사용자 1·5 선호 → A/B 풀목업(Clinical Calm vs Traffic-Light) → **B(Traffic-Light) 채택.** 디자인 언어: 흰 바탕 + 볼드 검정 구조(border-foreground 2px) + 고대비 verdict. **DANGER만 solid 빨강 알람 블록(흰 텍스트·큰 헤드라인)으로 소리치고, CAUTION/OK는 볼드테두리 흰 카드 + solid 아이콘**(위험만 강조=traffic-light 논리). 신규 NumbersStrip(combined/limit/+over, 초과 시만, 숫자는 verify()). 2단 레이아웃·label warnings·판정 코어 불변(시각만). 커밋 515667f. playwright로 danger/caution/ok 3종 렌더 검증. 미채택안(1 Clinical Calm 등)은 아티팩트에 보존. globals.css 토큰 기반이라 향후 방향 변경도 토큰+컴포넌트 className으로 가능.
